...
export default function ColaboradorView({ user, tickets, categories, onUpdate }: ColaboradorViewProps) {
    ...
    <div className="space-y-8">
      ...
      <div className="flex border-b border-gray-200">
        <button 
          ...
          className={`px-6 py-3 font-bold text-sm uppercase tracking-widest transition-all border-b-2 ${activeTab === 'status' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
        >
          Status (Ativos)
        </button>
        <button 
          onClick={() => setActiveTab('history')}
          className={`px-6 py-3 font-bold text-sm uppercase tracking-widest transition-all border-b-2 ${activeTab === 'history' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-400 hover:text-gray-600'}`}
        >
          Meu Histórico
        </button>
      </div>
