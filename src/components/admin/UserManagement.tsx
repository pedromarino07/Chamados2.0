...
export default function UserManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^[a-zA-Z0-9@.]+$/.test(formData.login)) {
      setError('O login deve conter apenas letras, números, @ ou .');
      return;
    }
      ...
      <div className="bg-white p-8 rounded-2xl border border-black/5 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold text-brand-gray">{editingId ? 'Editar Usuário' : 'Novo Usuário'}</h3>
          </div>
          {editingId && (
            <button 
              onClick={() => {
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-brand-orange outline-none transition-all"
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest">Login (Email ou Único)</label>
            <div className="relative">
              <UserIcon className="absolute left-3 top-3 w-5 h-5 text-gray-400" />
              <input
                type="text"
                ...
                onChange={(e) => setFormData({ ...formData, login: e.target.value.toUpperCase().replace(/[^A-Z0-9@.]/g, '') })}
                className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-brand-orange outline-none transition-all uppercase"
                placeholder="LOGIN"
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-brand-orange outline-none transition-all appearance-none bg-white"
              >
          ...
          <div className="md:col-span-2 lg:col-span-3 flex flex-col gap-4">
            ...
            {success && <p className="text-brand-orange text-sm font-bold bg-brand-orange/5 p-3 rounded-xl border border-brand-orange/10">{success}</p>}
            
            <button
              type="submit"
              className="bg-brand-orange text-white px-8 py-4 rounded-xl font-bold hover:bg-brand-orange/90 transition-all shadow-lg shadow-brand-orange/20 w-fit"
            >
            ...
            <tbody className="divide-y divide-gray-50">
              {users.map((u) => (
                <tr key={u.id} className="text-sm hover:bg-gray-50 transition-colors">
                  <td className="p-4 font-medium text-brand-gray">{u.name}</td>
                  ...
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase ${
                      ...
                      u.role === 'tecnico' ? 'bg-brand-orange/10 text-brand-orange' : 'bg-gray-50 text-gray-600'
                    }`}>
                      {u.role}
                    </span>
                  </td>
                  <td className="p-4 flex gap-2">
                    <button 
                      ...
                      className="flex items-center gap-1 px-3 py-1 bg-brand-blue/10 text-brand-blue rounded-lg hover:bg-brand-blue/20 transition-colors text-[10px] font-bold uppercase"
                    >
          ...
          {users.map((u) => (
            <div key={u.id} className="p-4 space-y-3">
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="font-bold text-brand-gray">{u.name}</h4>
